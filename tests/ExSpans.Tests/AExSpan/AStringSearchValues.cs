...
namespace System.Memory.Tests.ExSpan {
    public static class StringSearchValuesTests {
        ...
        public static void Values_ImplementsSearchValuesBase(StringComparison comparisonType, params string[] values) {
            ...
            AssertIndexOfAnyAndFriends(ExSpan<string>.Empty, -1, -1, -1, -1);
            ...
            void AssertIndexOfAnyAndFriends(ExSpan<string> values, int any, int anyExcept, int last, int lastExcept) {
                Assert.Equal(any >= 0, last >= 0);
                Assert.Equal(anyExcept >= 0, lastExcept >= 0);

                Assert.Equal(any, values.IndexOfAny(stringValues));
                Assert.Equal(any, ((ReadOnlyExSpan<string>)values).IndexOfAny(stringValues));
                Assert.Equal(anyExcept, values.IndexOfAnyExcept(stringValues));
                Assert.Equal(anyExcept, ((ReadOnlyExSpan<string>)values).IndexOfAnyExcept(stringValues));
                Assert.Equal(last, values.LastIndexOfAny(stringValues));
                Assert.Equal(last, ((ReadOnlyExSpan<string>)values).LastIndexOfAny(stringValues));
                Assert.Equal(lastExcept, values.LastIndexOfAnyExcept(stringValues));
                Assert.Equal(lastExcept, ((ReadOnlyExSpan<string>)values).LastIndexOfAnyExcept(stringValues));

                Assert.Equal(any >= 0, values.ContainsAny(stringValues));
                Assert.Equal(any >= 0, ((ReadOnlyExSpan<string>)values).ContainsAny(stringValues));
                Assert.Equal(anyExcept >= 0, values.ContainsAnyExcept(stringValues));
                Assert.Equal(anyExcept >= 0, ((ReadOnlyExSpan<string>)values).ContainsAnyExcept(stringValues));
            }
        }
        ...
        public static void IndexOfAny(StringComparison comparisonType, int expected, string text, string? values) {
            ExSpan<char> textExSpan = text.ToArray(); // Test non-readonly ExSpan<char> overloads

            string[] valuesArray = values is null ? ArrayHelper.Empty<string>() : values.Split(", ");
            ...
            Assert.Equal(expected, text.AsExSpan().IndexOfAny(stringValues));
            Assert.Equal(expected, textExSpan.IndexOfAny(stringValues));

            Assert.Equal(expected >= 0, text.AsExSpan().ContainsAny(stringValues));
            Assert.Equal(expected >= 0, textExSpan.ContainsAny(stringValues));
            ...
            void TestWithPoisonPages(PoisonPagePlacement poisonPlacement, int haystackLength) {
                ...
                ExSpan<char> haystack = memory.ExSpan;
                ...
                if (comparisonType == StringComparison.Ordinal) {
                    ...
                    asciiLetterLowerNotInSet = Enumerable.Range('a', 26).Select(c => (char)c)
                        .First(c => !values.AsExSpan().ContainsAny(c, char.ToUpperInvariant(c)));

                    asciiLetterUpperNotInSet = Enumerable.Range(0, 26).Select(c => (char)('Z' - c))
                        .First(c => !values.AsExSpan().ContainsAny(c, char.ToLowerInvariant(c)));
                }
            ...
            void TestWithDifferentMarkerChars(ExSpan<char> haystack, char marker) {
        ...
        public static void SimpleIndexOfAnyValues(params string[] valuesArray) {
            ...
            valuesArray[offset] = $"{original[0]}\u00F6{original.AsExSpan(1)}";
            TestCore(valuesArray);

            // Test null chars in values
            valuesArray[offset] = $"{original[0]}\0{original.AsExSpan(1)}";
            TestCore(valuesArray);
                ...
                foreach (StringComparison comparisonType in new[] { StringComparison.Ordinal, StringComparison.OrdinalIgnoreCase }) {
                    ...
                    for (int i = 0; i < text.Length - 1; i++) {
                        foreach (char replacement in "AaBb _!\u00F6") {
                            string newText = $"{text.AsExSpan(0, i)}{replacement}{text.AsExSpan(i + 1)}";
        ...
        private static int IndexOfAnyReferenceImpl(ReadOnlyExSpan<char> searchSpace, ReadOnlyExSpan<string> values, StringComparison comparisonType) {
        ...
        private sealed class StringSearchValuesTestHelper {
            public delegate int IndexOfAnySearchDelegate(ReadOnlyExSpan<char> searchSpace, ReadOnlyExSpan<string> values, StringComparison comparisonType);

            public delegate int SearchValuesSearchDelegate(ReadOnlyExSpan<char> searchSpace, SearchValues<string> values);
            ...
            public StringSearchValuesTestHelper(IndexOfAnySearchDelegate expected, SearchValuesSearchDelegate searchValues, int rngSeed = 42) {
                ...
                rng.NextBytes(MemoryMarshal.Cast<char, byte>(_randomChars.AsExSpan()));
            }
            ...
            private void Test(Random rng, ReadOnlyExSpan<char> haystackRandom, ReadOnlyExSpan<char> needleRandom) {
                string[] values = new string[rng.Next(MaxNeedleCount) + 1];

                for (int i = 0; i < values.Length; i++) {
                    ReadOnlyExSpan<char> valueExSpan;
                    do {
                        valueExSpan = GetRandomSlice(rng, needleRandom, MaxNeedleValueLength);
                    }
                    while (valueExSpan.Length < MinValueLength);

                    values[i] = valueExSpan.ToString();
                }
            ...
            private void Test(Random rng, StringComparison comparisonType, ReadOnlyExSpan<char> haystackRandom,
                string[] needle, SearchValues<string> searchValuesInstance) {
                ReadOnlyExSpan<char> haystack = GetRandomSlice(rng, haystackRandom, MaxHaystackLength);
            ...
            public static void AssertionFailed(ReadOnlyExSpan<char> haystack, string[] needle, SearchValues<string> searchValues, StringComparison comparisonType, int expected, int actual) {
