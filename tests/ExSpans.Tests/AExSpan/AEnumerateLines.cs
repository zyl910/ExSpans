...
namespace Zyl.ExSpans.Tests.AExSpan {
    public static partial class AEnumerateLines {
        ...
        public static void EnumerateLines_Empty() {
            ...
            var enumerator = ExSpan<char>.Empty.EnumerateLines().GetEnumerator();
            Assert.True(enumerator.MoveNext());
            Assert.Equal("", enumerator.Current.ToString());
            Assert.False(enumerator.MoveNext());

            enumerator = ReadOnlyExSpan<char>.Empty.EnumerateLines().GetEnumerator();
            Assert.True(enumerator.MoveNext());
            Assert.Equal("", enumerator.Current.ToString());
            Assert.False(enumerator.MoveNext());
        }
...
#nullable disable
        ...
        public static void EnumerateLines_Battery(string input, string[] expectedRanges) {
            ...
            static unsafe Range GetNormalizedRangeFromSubspan<T>(ReadOnlyExSpan<T> outer, ReadOnlyExSpan<T> inner) {
                // We can't use MemoryExtensions.Overlaps because it doesn't handle empty spans in the way we need.

                ref T refOuter = ref ExMemoryMarshal.GetReference(outer);
                ref T refInner = ref ExMemoryMarshal.GetReference(inner);
            ...
            static Range ParseRange(string input) {
                ...
                ReadOnlyExSpan<char> begin = input.AsExSpan(0, idxOfDots).Trim();
                Index beginIdx = (begin.IsEmpty) ? Index.Start : ParseIndex(begin);
                ReadOnlyExSpan<char> end = input.AsExSpan(idxOfDots + 2).Trim();
...
#nullable restore
        ...
        public static void EnumerateLines_EnumerationIsNotPolynomialComplexity(char newlineChar) {
            ...
            // n is the length of the span. See comments in ExSpanLineEnumerator.MoveNext and
            ...
            ExSpan<char> span = boundedMem.ExSpan;
            span.Fill('a');
            ...
            enumerator = ((ReadOnlyExSpan<char>)span).EnumerateLines().GetEnumerator();
            Assert.True(enumerator.MoveNext());
            Assert.Equal(512, enumerator.Current.Length);
        }
#endif // INTERNAL && TODO
