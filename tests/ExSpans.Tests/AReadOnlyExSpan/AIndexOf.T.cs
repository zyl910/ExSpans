    ...
    public static partial class AIndexOf {
        ...
        public static void MakeSureNoChecksGoOutOfRange() {
            ...
            for (int length = 0; length < 100; length++) {
                ...
                Assert.Equal(-1, new ReadOnlyExSpan<TInt>(a, GuardLength, length).IndexOf(new TInt(9999, checkForOutOfRangeAccess)));
                Assert.All(GetDefaultEqualityComparers<TInt>(), comparer => Assert.Equal(-1, new ReadOnlyExSpan<TInt>(a, GuardLength, length).IndexOf(new TInt(9999, checkForOutOfRangeAccess), comparer)));
            }
        }
        ...
        public static void TestNoMatchIndexOf_String() {
            ...
            for (int length = 0; length <= byte.MaxValue; length++) {
                ...
                Assert.Equal(-1, new ReadOnlyExSpan<string>(a).IndexOf(target));
                Assert.All(GetDefaultEqualityComparers<string>(), comparer => Assert.Equal(-1, new ReadOnlyExSpan<string>(a).IndexOf(target, comparer)));
            }
        }
