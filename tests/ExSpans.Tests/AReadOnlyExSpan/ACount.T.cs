...
namespace Zyl.ExSpans.Tests.AReadOnlyExSpan {
    public static partial class ACount {
        ...
        public static void ZeroLengthCount_Int() {
            Assert.Equal(0, ReadOnlyExSpan<int>.Empty.Count(0));
            Assert.All(GetDefaultEqualityComparers<int>(), comparer => Assert.Equal(0, ReadOnlyExSpan<int>.Empty.Count(0, comparer)));
        }

        [Fact]
        public static void ZeroLengthCount_RosInt() {
            for (int i = 0; i <= 2; i++) {
                Assert.Equal(0, ReadOnlyExSpan<int>.Empty.Count(new int[i]));
                Assert.All(GetDefaultEqualityComparers<int>(), comparer => Assert.Equal(0, ReadOnlyExSpan<int>.Empty.Count(new int[i], comparer)));
            }
        }
        ...
        public static void ZeroLengthNeedleCount_RosInt() {
            ...
            Assert.Equal(0, new ReadOnlyExSpan<int>(arr).Count(ReadOnlyExSpan<int>.Empty));
            Assert.All(GetDefaultEqualityComparers<int>(), comparer => Assert.Equal(0, new ReadOnlyExSpan<int>(arr).Count(ReadOnlyExSpan<int>.Empty, comparer)));
        }
        ...
        public static void ZeroLengthCount_String() {
            Assert.Equal(0, ReadOnlyExSpan<string>.Empty.Count("a"));
            Assert.All(GetDefaultEqualityComparers<string>(), comparer => Assert.Equal(0, ReadOnlyExSpan<string>.Empty.Count("a", comparer)));
        }

        [Fact]
        public static void ZeroLengthCount_RosString() {
            Assert.Equal(0, ReadOnlyExSpan<string>.Empty.Count(new[] { "a", "b" }));
            Assert.All(GetDefaultEqualityComparers<string>(), comparer => Assert.Equal(0, ReadOnlyExSpan<string>.Empty.Count(new[] { "a", "b" }, comparer)));
        }
        ...
        public static void TestNoMatchCount_String() {
            ...
            for (int length = 0; length <= byte.MaxValue; length++) {
                ...
                Assert.Equal(0, new ReadOnlyExSpan<string>(a).Count(target));
                Assert.All(GetDefaultEqualityComparers<string>(), comparer => Assert.Equal(0, new ReadOnlyExSpan<string>(a).Count(target, comparer)));
            }
        }
