...
namespace Zyl.ExSpans {
    /// <summary>
    /// Represents a contiguous region of memory, similar to <see cref="ReadOnlyExSpan{T}"/>.
    /// Unlike <see cref="ReadOnlyExSpan{T}"/>, it is not a byref-like type.
    /// </summary>
    ...
    public readonly partial struct ReadOnlyExMemory<T> : IEquatable<ReadOnlyExMemory<T>> {
        ...
        /// </exception>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ReadOnlyExMemory(T[]? array, int start, int length) {
            if (array == null) {
                if (start != 0 || length != 0)
        ...
        /// <param name="length">The number of items in the memory.</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal ReadOnlyExMemory(object? obj, int start, int length) {
        ...
        /// <summary>
        /// Defines an implicit conversion of an array to a <see cref="ReadOnlyExMemory{T}"/>
        /// </summary>
        public static implicit operator ReadOnlyExMemory<T>(T[]? array) => new ReadOnlyExMemory<T>(array);

        /// <summary>
        /// Defines an implicit conversion of a <see cref="ArraySegment{T}"/> to a <see cref="ReadOnlyExMemory{T}"/>
        /// </summary>
        ...
        /// <summary>
        /// Returns an empty <see cref="ReadOnlyExMemory{T}"/>
        /// </summary>
        ...
        /// </exception>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ReadOnlyExMemory<T> Slice(int start) {
        ...
        /// </exception>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ReadOnlyExMemory<T> Slice(int start, int length) {
#if TARGET_64BIT
            // See comment in Span<T>.Slice for how this works.
            ...
            if (tmpObject != null) {
                if (typeof(T) == typeof(char) && tmpObject is string s) {
                    ...
                    if (_index < 0) {
                        ...
                        void* pointer = Unsafe.Add<T>(Unsafe.AsPointer(ref ExMemoryMarshal.GetArrayDataReference(Unsafe.As<T[]>(tmpObject))), _index & RemoveFlagsBitMask);
