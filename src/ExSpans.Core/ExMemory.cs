...
namespace Zyl.ExSpans {
    ...
    /// <seealso cref="Span{T}"/>
    /// <seealso cref="ReadOnlyExMemory{T}"/>
    ...
    public readonly partial struct ExMemory<T> : IEquatable<ExMemory<T>> {
        ...
        public ExMemory<T> Slice(TSize start) {
            ...
            return new ExMemory<T>(_object, _index + start, _length - start);
        }
        ...
        private Span<T> GetSpanCore() {
            ...
            // However, we need to handle the case where a ReadOnlyExMemory<char> was created from a string
            // and then cast to a ExMemory<T>. Such a cast can only be done with unsafe or marshaling code,
        ...
        public unsafe MemoryHandle Pin() {
            ...
            // This needs to work, however, so that code that uses a single ExMemory<char> field to store either
            // a readable ReadOnlyExMemory<char> or a writable ExMemory<char> can still be pinned and
        ...
        public override bool Equals([NotNullWhen(true)] object? obj) {
            if (obj is ReadOnlyExMemory<T>) {
                return ((ReadOnlyExMemory<T>)obj).Equals(this);
