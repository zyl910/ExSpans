...
namespace Zyl.ExSpans.Buffers {
    /// <summary>
    /// A memory manager that supports automatic memory reallocation and alignment. When the length is less than <see cref="AbstractAllocExMemoryManager{T}.MaxArrayLength">MaxArrayLength</see>, it uses array Pooling; otherwise, it uses native memory
    /// (支持自动内存重新分配和对齐的内存管理器. 当长度小于 <see cref="AbstractAllocExMemoryManager{T}.MaxArrayLength">MaxArrayLength</see> 时它使用数组池，否则它就使用原生内存).
    /// </summary>
    /// <typeparam name="T">The element type (元素的类型).</typeparam>
    public class ReallocExMemoryManager<T> : AbstractReallocExMemoryManager<T>, IDisposable where T : unmanaged {
...
#pragma warning disable CA2015
        ...
        // /// </remarks>
        // ~ReallocExMemoryManager() {
        //     Dispose(false);
        // }
#pragma warning restore CA2015

        /// <summary>
        /// Create ReallocExMemoryManager. It contains parameters <paramref name="Pool"/>, <paramref name="length"/>, <paramref name="alignment"/>, <paramref name="flags"/>, <paramref name="maxArrayLength"/>, <paramref name="capacity"/>, <paramref name="onMeasureCapacity"/>.
        /// </summary>
        ...
        public ReallocExMemoryManager(ArrayPool<T>? Pool, TSize length, TSize alignment = 0, MemoryAllocFlags flags = default, TSize maxArrayLength = 0, TSize capacity = 0, MeasureCapacityFunc? onMeasureCapacity = null)
            : base(Pool, length, alignment, flags, maxArrayLength, capacity, onMeasureCapacity) {
        }

        /// <summary>
        /// Create ReallocExMemoryManager. It contains parameters <paramref name="length"/>, <paramref name="alignment"/>, <paramref name="flags"/>, <paramref name="maxArrayLength"/>, <paramref name="capacity"/>, <paramref name="onMeasureCapacity"/>.
        /// </summary>
        /// <param name="length">Length of data (数据的长度).</param>
        /// <param name="alignment">The alignment value (in bytes) of the memory block. This must be a power of <c>2</c>. When it is 1, it means no alignment is required. When it is 0, use the previous value (内存块的对齐值（以字节为单位）. 这必须是 2的幂. 为 1时表示无需对齐.为0时使用上一次的值).</param>
        /// <param name="flags">Memory alloc flags (内存分配标志).</param>
        ...
        /// <param name="onMeasureCapacity">Function for measure capacity (测量容量的函数).</param>
        /// <exception cref="ArgumentOutOfRangeException">The length parameter must be greater than or equal to 0. The length parameter out of array max length.</exception>
        public ReallocExMemoryManager(TSize length, TSize alignment = 0, MemoryAllocFlags flags = default, TSize maxArrayLength = 0, TSize capacity = 0, MeasureCapacityFunc? onMeasureCapacity = null)
            : this(ArrayPool<T>.Shared, length, alignment, flags, maxArrayLength, capacity, onMeasureCapacity) {
        }

        /// <summary>
        /// Create ReallocExMemoryManager. It contains parameters <paramref name="length"/>.
        /// </summary>
        /// <param name="length">Length of data (数据的长度).</param>
        /// <exception cref="ArgumentOutOfRangeException">The length parameter must be greater than or equal to 0. The length parameter out of array max length.</exception>
        public ReallocExMemoryManager(TSize length)
            : this(length, 0) {
        }
